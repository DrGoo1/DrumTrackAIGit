...
import { 
  ...
  Plus, Minus, RefreshCw, RotateCcw, Edit
} from 'lucide-react';
...
const WaveformDisplay = ({ audioData, color, isPlaying, currentTime }) => {
  const canvasRef = useRef(null);
  const [waveformData, setWaveformData] = useState(null);

  useEffect(() => {
    if (audioData && audioData.audioUrl) {
      generateWaveformFromUrl(audioData.audioUrl);
    }
  }, [audioData]);
    ...
    try {
      ...
      audio.onloadedmetadata = async () => {
        try {
          ...
          console.log('✓ Waveform generated for', audioUrl);
        } catch (error) {
          console.error('Error generating waveform:', error);
          // Fallback to demo waveform
          const demoWaveform = Array.from({ length: 200 }, (_, i) => {
            const x = i / 200;
            return Math.sin(x * Math.PI * 8) * Math.exp(-x * 2) * (Math.random() * 0.3 + 0.7);
          });
          setWaveformData(demoWaveform.map(Math.abs));
        }
      };
      
      audio.onerror = () => {
        console.warn('Failed to load audio, using demo waveform');
        // Fallback to demo waveform
        const demoWaveform = Array.from({ length: 200 }, (_, i) => {
          const x = i / 200;
          return Math.sin(x * Math.PI * 8) * Math.exp(-x * 2) * (Math.random() * 0.3 + 0.7);
  ...
  }, [waveformData, isPlaying, currentTime]);
  ...
  const drawWaveform = () => {
    ...
    waveformData.forEach((amplitude, index) => {
      ...
      const progress = currentTime / (audioData?.duration || 30);
      const isPlayed = index / waveformData.length < progress;
      
      ctx.fillStyle = isPlayed && isPlaying ? '#ffffff' : color;
      ctx.fillRect(x, y, barWidth - 1, barHeight);
    });

    // Draw playback position indicator
    if (isPlaying && audioData?.duration) {
      const progress = currentTime / audioData.duration;
...
// Drum Studio Component
const DrumStudio = ({ onClose, onPatternCreate }) => {
  const [selectedKit, setSelectedKit] = useState('acoustic');
  const [bpm, setBpm] = useState(120);
  const [isPlaying, setIsPlaying] = useState(false);
  const [currentStep, setCurrentStep] = useState(0);
  
  // 16-step pattern for each drum type
  ...
  const drumKits = {
    ...
    electronic: {
      name: 'Electronic Kit',
      samples: {
        kick: 'Electronic Kick',
        snare: 'Electronic Snare',
        hihat: 'Electronic Hi-Hat',
        openhat: 'Electronic Open Hat',
        crash: 'Electronic Crash',
        ride: 'Electronic Ride'
      },
      colors: {
        kick: '#7C3AED',
        snare: '#EC4899',
  ...
  const clearPattern = (drumType) => {
    setPatterns(prev => ({
      ...prev,
      [drumType]: Array(16).fill(false)
    }));
  };

  const randomizePattern = (drumType) => {
    setPatterns(prev => ({
      ...prev,
      [drumType]: Array(16).fill().map(() => Math.random() > 0.7)
    }));
  };
  ...
  const loadPreset = (presetName) => {
    const presets = {
      ...
      'breakbeat': {
        ...
        snare: [false, false, false, false, true, false, false, true, false, false, true, false, false, false, false, false],
        hihat: [true, true, false, true, true, true, false, true, true, false, true, true, true, false, true, true]
      }
    };
    
    if (presets[presetName]) {
      setPatterns(prev => ({ ...prev, ...presets[presetName] }));
    }
  };
  ...
  const exportPattern = () => {
    ...
    console.log('Exported drum pattern:', patternData);
  };

  return (
    <div className="h-96 bg-gradient-to-r from-purple-900 to-gray-900 border-t border-purple-600 overflow-y-auto">
      <div className="p-4">
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div className="text-2xl">🥁</div>
            <h3 className="text-xl font-bold text-white">Drum Studio</h3>
            <span className="text-sm text-purple-300">Pattern Sequencer</span>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <input
                ...
                className="w-16 bg-gray-800 text-white rounded px-2 py-1 text-sm border border-gray-600"
                min="60"
                max="200"
              />
            </div>
        ...
        <div className="space-y-3">
          {Object.entries(patterns).map(([drumType, pattern]) => (
            <div key={drumType} className="bg-gray-800 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-3">
                  <div 
                    className="w-4 h-4 rounded"
                    style={{ backgroundColor: drumKits[selectedKit].colors[drumType] }}
                  ></div>
                  <span className="font-medium text-white capitalize">
                    {drumKits[selectedKit].samples[drumType]}
                  </span>
                </div>
                
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => randomizePattern(drumType)}
                    className="text-xs bg-blue-600 hover:bg-blue-700 px-2 py-1 rounded"
                  >
                    Random
                  </button>
                  <button
                    onClick={() => clearPattern(drumType)}
              ...
              <div className="flex space-x-1">
                {pattern.map((step, index) => (
                  <button
                    key={index}
                    onClick={() => toggleStep(drumType, index)}
                    ...
                    style={{
                      backgroundColor: step ? drumKits[selectedKit].colors[drumType] : undefined
                    }}
                  >
                    {index + 1}
                  </button>
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-700">
          <div className="flex items-center space-x-3">
            ...
            <button
              onClick={() => loadPreset('basic-rock')}
              className="text-xs bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded"
            >
              Basic Rock
            </button>
            <button
              onClick={() => loadPreset('funk-groove')}
              className="text-xs bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded"
            >
              Funk Groove
            </button>
...
const DrumTracKAIWebDAW = ({ audioAnalysis, onBack, stemData }) => {
  const [isPlaying, setIsPlaying] = useState(false);
  ...
  // Load stems when stemData is provided
  useEffect(() => {
    if (stemData && Object.keys(stemData).length > 0) {
      console.log('Loading stem data into WebDAW:', stemData);
      
      const stemTracks = Object.entries(stemData).map(([stemType, audioData], index) => {
        const colors = ['#EF4444', '#F59E0B', '#10B981', '#8B5CF6', '#EC4899', '#06B6D4'];
        return {
          id: stemType.toLowerCase(),
      ...
      setTracks(prevTracks => [
        prevTracks[0], // Keep master track
        ...stemTracks
      ]);
      
      console.log('Loaded stems into tracks:', stemTracks.map(t => t.name));
    } else {
      // Fallback to default stems if no stemData provided
      const defaultStems = [
  ...
  return (
    <div className="h-screen bg-gray-900 text-white flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2 bg-gray-700 hover:bg-blue-600 rounded-lg"
            >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col bg-gray-900">
          ...
          <div className="flex-1 bg-gray-900 p-4 overflow-y-auto">
            {tracks.filter(t => !t.isMaster).length > 0 ? (
              <div className="space-y-4">
                {tracks.filter(t => !t.isMaster).map((track, index) => (
                  <div key={track.id} className="bg-gray-800 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="text-xs text-gray-400">
                        {track.audioData?.duration ? `${track.audioData.duration}s` : 'Loading...'}
                      </div>
                    </div>
                    
                    {/* Waveform Canvas */}
                    <div className="bg-gray-900 rounded-lg p-3 h-20 flex items-center">
                      {track.audioData ? (
                        <WaveformDisplay 
                    ...
                    <div className="flex items-center justify-between mt-3 text-xs">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-white">{track.volume}%</span>
                        <span className="text-gray-400 ml-2">Pan:</span>
                        <span className="text-white">
                          {track.pan > 0 ? `R${track.pan}` : track.pan < 0 ? `L${Math.abs(track.pan)}` : 'C'}
                        </span>
                      </div>
                      <div className="flex items-center space-x-1">
                        {track.muted && <span className="bg-red-600 px-1 rounded text-xs">MUTE</span>}
                        {track.solo && <span className="bg-yellow-600 px-1 rounded text-xs">SOLO</span>}
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-2xl font-bold mb-2">No Stems Loaded</h3>
                <p className="text-gray-400">
                  Load audio stems to see waveform visualization
                </p>
              </div>
            )}
          </div>
      ...
      <EditDrumsModal
        ...
        jobId={audioAnalysis?.job_id || 'demo_song_01'}
...
export default DrumTracKAIWebDAW;
